...
namespace Common
{
    internal class ChineseSpell
    {
        ...
        /// <returns></returns>
        public static string ChangChineseSpell(string strText)
        {
            if (strText == null || strText.Length == 0)
                return strText;
            strText = strText.Insert(1, " ");
            System.Text.StringBuilder myStr = new System.Text.StringBuilder();
            foreach (char vChar in strText)
            {
                // 若是字母则直接输出
                ...
                if ((int)vChar >= 19968 && (int)vChar <= 40869)
                {
                    ...
                    foreach (string strList in ChineseCharList)
                    {
                        if (strList.IndexOf(vChar) > 0)
                        {
                            myStr.Append(strList.Substring(0, strList.IndexOf(',')));
                            break;
                        }
